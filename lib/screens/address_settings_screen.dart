...
class _AddressSettingsScreenState extends State<AddressSettingsScreen> {
  ...
  Future<void> _loadAddressSettings() async {
    ...
    // 修正: setState()を追加し、UIを更新する
    if (mounted) {
      setState(() {
        _selectedPrefecture = savedPrefecture;
        if (_selectedPrefecture != null) {
          try {
            _selectedRegion = AddressService.getRegions().firstWhere(
                    (region) => AddressService.getPrefecturesForRegion(region).contains(_selectedPrefecture!)
            );
          } catch (e) {
                ...
                children: [
                  ...
                  Card(
                    child: Padding(
                      ...
                      child: DropdownButtonFormField<String>(
                        // value: _selectedRegion, // ここは`FutureBuilder`が完了した時点で値が設定されている
                  ...
                  Card(
                    child: Padding(
                      padding: const EdgeInsets.all(16.0),
                      child: DropdownButtonFormField<String>(
                        // value: _selectedPrefecture, // ここも同様
